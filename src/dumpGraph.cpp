...
void dump(const std::string& dump_directory,
  ...
  for(int i = 0; i < surf_cloud_keyframes.size(); i++) {
    ...
    int32_t stamp_sec = static_cast<int32_t>(std::floor(keyframe_stamps[i]));
    uint32_t stamp_nsec = static_cast<uint32_t>((keyframe_stamps[i] - stamp_sec) * 1e9);

    std::ofstream data_ofs(keyframe_directory + "/data");
    data_ofs << "stamp " << stamp_sec << " " << stamp_nsec << "\n";
